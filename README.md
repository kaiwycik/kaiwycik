# Hi, I'm @kaiwycik 👋

My work sits at the intersection of statistical learning, data-driven scientific discoveries, and recently tensor methods — with a broad interest in machine learning research and its foundations.

---

## Education

- 🎓 **Ph.D., Statistics & Operations Research** — UNC Chapel Hill (candidate)
- 🎓 **M.A., Statistics** — Columbia University
- 🎓 **B.Sc., Information Systems** — University of Cologne

---

## Research Interests

- Statistical learning & machine learning
- Unsupervised machine learning and its validation
- High-dimensional and selective inference
- Tensor methods 

---

## Links

- 📄 [Recent Work](https://arxiv.org/abs/2606.00327)
- 💼 [LinkedIn](https://www.linkedin.com/in/kai-wycik-194120210/)
- ✉️ [kwycik@unc.edu](#)
- ✉️ [kai.wycik@columbia.edu](#)

---

*Open to research collaborations and conversations about statistics, ML, and AI.*
